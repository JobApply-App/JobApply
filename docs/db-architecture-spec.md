# אפיון ארכיטקטורת מסד הנתונים — מ-26 טבלאות ל-23
...
| 2 | **אותה ישות מיוצגת פעמיים** — נבדק פר-מקרה; אושר רק לכישורים | 36 כישורים ב-`cv_claims` מול 121 ב-`profile_entities`, **0 קישור**. שאר טענות הכפילות לא שרדו בדיקה — §4.1-4.3 |
...
**היעד:** 23 טבלאות, כל אחת עם ייעוד יחיד, כולן תלויות ב-`profiles.id` כעוגן, ללא כפילות מושגית.
...
### עיקרון 6 — כל קריאת פרופיל מקבלת `user_id`, ונכשלת ריק

התגלה תוך כדי ביצוע שלב 2, ולא היה במסמך המקורי — הוא עוסק בסכימה, אבל הבעיה החמורה ביותר שנמצאה הייתה **בקוד שקורא ממנה**.

הסינגלטון `USER_PROFILE` ב-`user_profile.py` הוא פרופיל אמיתי (11KB). מה בדיוק דולף ממנו תלוי בסביבה, וההבחנה חשובה:

| נתון | מקור | דולף היכן |
|------|------|-----------|
| **7 מעסיקים, 2 תארים, נרטיבים** | ליטרלים בקוד המקור | **בכל סביבה** |
| טלפון, מיקום, אימייל | `backend/personal_overrides.json` (מוחרג מ-git, נטען בזמן import) | רק במכונת מפתח או בדיפלוי שנושא את הקובץ |

ארבעה נתיבי קוד חיים קראו ממנו במקום מהמשתמש הקורא:

| מיקום | מה קרה | חומרה |
|-------|--------|--------|
| `chat.py::_build_system_prompt` | הזריק את הסינגלטון המלא לכל שיחת Ariel, בתווית `"verified — treat as ground truth"`. ה-endpoint **מאומת** ויש בו `user.user_id` — הוא פשוט לא הועבר | כל משתמש קיבל את היסטוריית הקריירה של אדם אחר |
| `master_profile_service._empty_profile()` | זרע כל פרופיל חדש עם `personal` של הסינגלטון | משתמש חדש נוצר עם פרטי קשר של מישהו אחר — בסביבות שנושאות את קובץ ה-overrides |
| `profile_interviewer._build_profile_context` | קרא ניסיון והשכלה מהסינגלטון, בהנחה של `user_id='default'` — מצב שבוטל ב-Phase 1 | הראיון תיאר קריירה של אדם אחר |
| `user_profile.resolve_profile()` | החזיר את הסינגלטון בכל שגיאת DB | דליפה בנתיב שגיאה, בתוך יצירת קו"ח |

**הכלל:** פונקציה שמחזירה נתוני פרופיל חייבת לקבל `user_id`, ובכשל להחזיר `{}` — לעולם לא ברירת מחדל גלובלית. `resolve_profile` נכשלת ריק היום; הנימוק הישן ("קו"ח מפרופיל ישן עדיף על 500") היה תקף רק כשהיה משתמש אחד והסינגלטון היה אותו אדם.

**הכלל המשלים:** קבוע גלובלי שנראה כמו נתוני משתמש הוא סכנה כל עוד הוא ניתן לייבוא. ארבעת הייבואים של `USER_PROFILE` שנותרו ללא שימוש הוסרו, כדי שהשימוש החוזר הבא ידרוש החלטה מפורשת ולא ישלים אוטומטית.

### עיקרון 7 — פרומפט וקבוע לוגי משותפים לכל המשתמשים; עובדה אישית בתוכם היא באג

התגלה בשלושה גלים, כל אחד עמוק מהקודם, וכל אחד נתפס רק כי חיפשתי שוב:

| גל | היכן | מה היה שם |
|----|------|-----------|
| 1 | `outreach_service.py` | תחום הקריירה והמעסיק מוקשחים בהודעות שנשלחות למגייסים אמיתיים |
| 2 | `tailor.py` | **טבלת דוקטרינה פר-מעסיק** — "GO-OUT ... Always gets primary slot. Lead with scope: 800+ clients, 7 direct reports"; רשימת מעסיקים מוחרגים בשמם; `Seats.io` בתוך רשימת ה-ALLOW של PM |
| 3 | `researcher.py`, `cv_aggregator_service.py` | **לוגיקה, לא פרומפט:** `_HIGH_IMPACT_MARKERS = {"go-out",...}` סימן חברה אחת כבעלת-השפעה עבור כל משתמש; `_SAAS_SIGNALS` הכיל שם חברה |

**למה `sed` הוא התשובה הלא נכונה כאן:** החלפת `"Ron's"` ב-`"the candidate's"` תוך השארת `"800+ clients, 7 direct reports"` מייצרת פרומפט שנקרא גנרי ומתנהג שגוי — ו**קשה יותר לאתר** מהמצב הקודם. הכלל: מחליפים את **הכללים**, לא את התוויות.

הדוקטרינה עצמה הייתה טובה וניתנת להכללה במלואה. טבלת המעסיקים הוחלפה בחמישה כללים (`C1`–`C5`) שהמודל מיישם על הפרופיל שכבר מוזרק אליו: הובל עם היקף מכומת, דרג תפקידים לפי חפיפת מילים ל-JD ולא לפי עדכניות, הפשט ז'רגון תחומי שה-JD לא חולק, התקדמות היא אות בפני עצמה, והסמכות עוקבות אחר ארכיטיפ ה-JD.

**אימות:** הרצתי את הפרומפט מול פרופיל בדיוני מתחום הסיעוד. הפלט השתמש במעסיקים של הפרופיל (Rambam, Clalit), החריג נכון תפקיד חסר-סיגנל בלי שנקוב בשמו, שימר את כל 8 המדדים — ו**לא הזכיר אף מעסיק מהפרומפט הישן, ולא את הדוגמאות הסינתטיות החדשות שלי** (הוראת "study the SHAPE, never the values" עבדה).

**הבדיקה שנוספה תפסה מה שהגרפים שלי החמיצו:** `Go-Out` בכתיב Title-Case ב-`copilot.py`, כי חיפשתי `GO-OUT` באותיות גדולות. `test_profile_singleton_isolation.py::TestPromptsCarryNoPersonalFacts` משווה case-insensitive על כל הפרומפטים הנקובים.
...
### 2.3 שלוש טבלאות שיחה — **חשד לכפילות שלא אושר**

| טבלה | שורות | מה היא מייצגת | עמודות ייעודיות |
|------|-------|----------------|------------------|
| `chat_sessions` | 14 | שיחה חופשית עם Ariel | 0 |
| `ariel_sessions` | 34 | מפגש חקירה יזום | 5 |
| `profile_interviews` | 17 | ראיון onboarding מובנה | 5 |

**0 `session_id` משותפים בין כל זוג** — מה שהוביל אותי במקור למסקנה "שלוש ממגורות של אותו מושג". בדיקה מעמיקה יותר הפריכה זאת: אין `session_id` משותף כי אלה **תהליכים שונים**, לא כי מישהו פיצל תהליך אחד. מחזורי חיים שונים, אפס צרכנים משותפים, ושניים מהשלושה דורשים 5 עמודות ייעודיות. הפירוט והמסקנה — §4.3.

החשש היחיד שכן אושר: פיצ'ר שיחה רביעי אכן יוליד טבלה רביעית אם לא נקבע כלל. הכלל נקבע ב-§4.3.
...
| 17 | `chat_sessions` | ✅ נשארת | תהליך נפרד — §4.3 |
| 18 | `ariel_sessions` | ✅ נשארת | תהליך נפרד — §4.3 |
| 19 | `profile_interviews` | ✅ נשארת | תהליך נפרד — §4.3 |
...
**סיכום: 4 טבלאות מתמזגות → 23 טבלאות.**
...
### 4.3 שלוש טבלאות השיחה — **לא מתמזגות** `[✅ הוכרע · מיגרציה 32d536527e9a]`

**זה הסעיף שהכי השתנה אחרי בדיקה.** הוא הציע טבלת `conversations` אחת עם דיסקרימינטור. הראיות שוללות את זה:

| | `chat_sessions` | `ariel_sessions` | `profile_interviews` |
|---|---|---|---|
| מחזור חיים | פתוח, **לעולם לא נגמר** — אין `status`, אין `ended_at` | ממוקד-יעד: `session_type`, `ariel_goal`, `confidence_delta_total`, `ended_at` | מכונת מצבים מתכנסת: `draft_profile`, `confidence_map`, `pending_probes` |
| עמודות ייעודיות | 0 | **5** | **5** |
| צרכנים | `history.py`, חילוץ persona | `ariel.py`, `profile_update_service` | ה-repository שלה בלבד |

**שני כשלים בהצעה המקורית:**

1. **היא סותרת את הגבול שהיא עצמה קבעה.** הסעיף קבע ש"סוג שדורש יותר מ-4 עמודות ייעודיות מקבל טבלת לוויין ולא מנפח את הראשית" — אבל שניים מהשלושה דורשים 5 **כבר עכשיו**. הטבלה המאוחדת הייתה ~16 עמודות שבכל שורה לפחות 5 מהן NULL.
2. **אין צרכן משותף.** אף מודול לא קורא יותר מטבלה אחת, ושום מקום בקוד לא מבקש מבט מאוחד על "כל השיחות". המיזוג לא היה מפשט אף שאילתה — רק מוסיף NULL-ים ודיסקרימינטור.

אלה שלושה **תהליכים** שונים שבמקרה כולם שומרים הודעות, לא שלושה עותקים של דבר אחד. לפי עיקרון 3 (מחזור חיים שונה) — הפרדתם מוצדקת.

**החשש שהסעיף כן זיהה נכון** — "פיצ'ר שיחה רביעי יוליד טבלה רביעית" — נשאר תקף. הכלל שנקבע במקומו: **סוג שיחה חדש מרחיב את `conversation_events` או משתמש בטבלת מפגש קיימת; טבלת מפגש חדשה דורשת הצדקה מפורשת מול עיקרון 3.**

### 4.3.1 מה כן מוזג: `ariel_probe_log` → `conversation_events`

זה המיזוג היחיד בשלב 5 שהראיות תומכות בו. שניהם **אירוע בתוך אותו `ariel_session`**: שניהם FK ל-`ariel_sessions(session_id)` ול-`profiles(user_id)`, שניהם מזינים את Confidence Matrix, ושניהם ריקים. סוג ה-probe דורש בדיוק שתי עמודות ייעודיות (`probe_outcome`, `entity_id`) — הרבה בתוך הגבול.

**באג שנתפס באימות, לפני הבדיקות:** `extracted_entity_ids` ו-`extraction_confidence` היו `NOT NULL` כי כל שורה הייתה STAR. לשורת probe אין אותם. הפכתי אותם ל-nullable — **ומיד החלפתי את מה שאיבדנו באילוץ שאוכף דרישות פר-סוג:**

```sql
CHECK ( (event_kind = 'star_extraction' AND extracted_entity_ids IS NOT NULL
                                        AND extraction_confidence IS NOT NULL)
     OR (event_kind = 'probe'           AND entity_id IS NOT NULL) )
```

זה הלקח הכללי: **הוספת סוג שני לטבלה מרפה אילוצים שהיו נכונים לסוג היחיד. הדיסקרימינטור חייב לקחת על עצמו את השלמות שהם סיפקו** — אחרת המיזוג פותח דלת לשורות פגומות משני הסוגים. אומת ששני המקרים נדחים בפועל.

**נתיב ה-cooldown נשמר:** האינדקס `(user_id, entity_id, probed_at DESC)` שגיבה את שאילתת "אל תחקור ישות שנחקרה ב-48 השעות האחרונות" הוחלף באינדקס חלקי בעל אותה צורה מעל שורות ה-probe בלבד.

---

### 4.3-הצעה-מקורית (לא בוצעה) — `conversations` עם דיסקרימינטור
