...
| 3 | **מינוח אישי במקום גנרי** — `why_ron` כשם עמודה במוצר רב-משתמשי | 106 שימושים ב-Backend, **0 ב-Frontend** (שינוי בטוח) |
...
**היקף:** 106 שימושים ב-Backend, **0 ב-Frontend** — שינוי פנימי לחלוטין, ללא שבירת API. בנוסף:

| נוכחי | יעד |
|-------|-----|
| `why_ron` (עמודה) | `fit_brief` |
